...
| 3 | Send Streaming Message (SSE) | `sendStreamingMessage` | helloworld, adk_currency_agent (v0.3) | `interop_test.bal::testInteropSendMessageStream`, `currency_agent_interop_test.bal::testCurrencyAgentSendMessageStream` | ✅ passing — full event sequence: task created (`SUBMITTED`) → `WORKING` status → artifact update → `COMPLETED` status → clean stream close |
...
# interactive discovery + sendMessage + sendStreamingMessage + multi-turn walkthrough
