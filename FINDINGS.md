...
| dice_agent | Java/Quarkus, on Claude via LangChain4j (`a2a-samples/samples/java/agents/dice_agent_multi_transport`) | [servers/dice_agent/findings.md](servers/dice_agent/findings.md) — the only reference agent that genuinely serves REST (HTTP+JSON) and gRPC, not just JSON-RPC; needed an `a2a-java-sdk` 0.3.2.Final→1.1.0.Final migration to get a spec-correct `supportedInterfaces` card field; `ballerina/a2a`'s real `Client` confirmed working end-to-end over all three transports (`bal test --sticky --groups interop`), each correctly reaching a real, placeholder-key Anthropic 401 — a real key away from 3 passing tests |
...
full story. All three transports were verified end-to-end, both directly
(gRPC via the sample's own Java `TestClient`, REST and JSON-RPC via raw
`curl`) and via `ballerina/a2a`'s own real `Client`
(`tests/dice_agent_interop_test.bal`, `bal test --sticky --groups interop`)
— every path correctly reaches a genuine `401` from a placeholder Anthropic
key, confirming the request/task-creation/error-decoding pipeline is
correct on every transport. Getting the real client running live required
two fixes, both documented in `servers/dice_agent/findings.md` §6-7: a
`ballerina/http`/`ballerina/grpc` binary version skew, and Ballerina's
default HTTP/2 client not negotiating h2c correctly against this agent's
Quarkus dev-mode server. Not yet closed for real: no `ANTHROPIC_API_KEY`
was available this session to confirm an actual successful (non-401)
response — a real key away from 3 passing tests, no further code changes
expected. Two things a mock genuinely cannot settle and a real server can,
per the gRPC design spec, also remain open: whether A2A servers populate
`google.rpc.ErrorInfo` in their gRPC status details, and whether
`A2A-Version`/`A2A-Extensions` are honoured as gRPC metadata by real
implementations.
