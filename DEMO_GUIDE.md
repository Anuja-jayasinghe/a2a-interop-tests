...
`Client` talking to real, independently-built A2A reference agents that
...
| `langgraph` currency agent (Python/LangGraph, `a2a-samples`, on Claude) | **v0.3** | The one agent in this repo that processes tasks asynchronously enough to genuinely test `cancelTask`/`subscribeToTask` mid-flight (not just against an already-terminal task), plus real push-notification config CRUD and genuine `INPUT_REQUIRED`/multi-turn continuation — see §3's Claude setup and `servers/langgraph_agent/` |
...
`helloworld` is the simpler, faster baseline to sanity-check first. If
you're specifically validating the operations `helloworld` and
`adk_currency_agent` can't genuinely exercise (real in-flight cancel/
subscribe, push notifications), see the `langgraph` agent section below.
...
### `langgraph` currency agent (v0.3, Claude only in this repo) — genuine cancel/subscribe/push notifications
...
them can only ever be tested against an already-terminal task. The
`langgraph` currency agent (`a2a-samples/samples/python/agents/langgraph`
— a different sample from `adk_currency_agent`, built on LangGraph rather
than Google ADK) makes a real, several-second Claude call plus a real tool
call per turn, which is enough time for a `cancelTask` or
`subscribeToTask` sent immediately after to hit a task that is genuinely
still running. It also genuinely wires push-notification support
(`capabilities.pushNotifications: true`, a real
`InMemoryPushNotificationConfigStore` + `BasePushNotificationSender`) —
`helloworld` doesn't support the capability at all.
...
Then, same pattern as every other agent here:
...
Listens on `http://localhost:10000` by default. Run its interop tests:
...
With both servers running, from the repo root (this is a single Ballerina
package — `tests/` is not its own package, so run `bal test` from here, not
from inside `tests\`):
...
- **Currency agent exits immediately** — `GOOGLE_API_KEY` isn't set. Check
  `.env` has a real key and that step 3's `set GOOGLE_API_KEY=...` actually
  ran in the *same* `cmd` window, right before `uv run currency_agent` —
  `set` only affects the window it's typed in, so a fresh window (or one
  where you set it before switching directories with `cd /d`) won't have
  it. Confirm with `echo %GOOGLE_API_KEY%` before running.
...
- **Port already in use (9999 or 10999)** — a previous run's server is
  still alive in the background. Find and stop it:
