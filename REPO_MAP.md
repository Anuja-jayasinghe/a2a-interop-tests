...
| `demo_agent/` | The headline demo, and a separate Ballerina project from `demo/`: a real client-side A2A agent. Given a question, it self-assesses whether it can answer locally, and if not, discovers candidate agents via the spec's well-known-URI mechanism (§8.2), picks one by reasoning over its declared skills, delegates over streaming, and presents both the remote agent's verbatim reply and its own synthesized answer. Two run modes sharing one implementation: `bal run --sticky` (interactive) and `bal run --sticky -- scripted` (six fixed scenarios, unattended). See `demo_agent/README.md`. |
